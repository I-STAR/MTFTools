classdef EsfCalc_WedgeVAng < mtf.SfCalc
% angle is not hard coded --> but change based on p1 (fitted)
...
methods
  function obj = EsfCalc_WedgeVAng(varargin)
  ...
  function [] = testFit(o, u)
    ...
    assert(abs(atand(o.p1)) <= 45);
    figure, plot(p2, '-*');
    title(['ang: ', num2str(atand(o.p1))]);
  end
  function [esf, esfAxs] = apply(o, u, varargin)
    [u] = permuteCrop(o, u);
    [esf, esfAxs] = mtf.oversampleCurves(u, o.locs, abs(atand(o.p1))); % so now we are in mm...
  end
  ...
  function [esfCel, esfAxsCel] = applyMult(o, u, nBin, varargin)
    ...
    for i = 1:nBin
      [esfCel{i}, esfAxsCel{i}] = mtf.oversampleCurves(uCel{i}, locCel{i}, abs(atand(o.p1)));
    end  
  end
